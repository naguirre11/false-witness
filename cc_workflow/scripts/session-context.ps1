...
# --------------------------------------------------------------
# 4. Last Full Test Validation
# --------------------------------------------------------------
Write-Host "[LAST FULL TEST]" -ForegroundColor Yellow
$fullTestTrackingFile = "$projectRoot\.last_full_test.json"

if (Test-Path $fullTestTrackingFile) {
    try {
        $fullTestData = Get-Content $fullTestTrackingFile -Raw | ConvertFrom-Json
        $lastFullTestCommit = $fullTestData.commit
        $lastFullTestCommitShort = $fullTestData.commit_short
        $lastFullTestDate = $fullTestData.date

        Write-Host "  Date: $lastFullTestDate"
        Write-Host "  Commit: $lastFullTestCommitShort"
        $envVars["SESSION_FULL_TEST_DATE"] = $lastFullTestDate
        $envVars["SESSION_FULL_TEST_COMMIT"] = $lastFullTestCommitShort

        # Count commits since last full test
        Push-Location $projectRoot
        $commitsSince = git rev-list --count "$lastFullTestCommit..HEAD" 2>$null
        Pop-Location

        if ($commitsSince -eq 0) {
            Write-Host "  Commits since: 0 (up to date)" -ForegroundColor Green
            $envVars["SESSION_FULL_TEST_COMMITS_SINCE"] = "0"
            $envVars["SESSION_FULL_TEST_STATUS"] = "CURRENT"
        } elseif ($commitsSince -le 3) {
            Write-Host "  Commits since: $commitsSince" -ForegroundColor Yellow
            $envVars["SESSION_FULL_TEST_COMMITS_SINCE"] = "$commitsSince"
            $envVars["SESSION_FULL_TEST_STATUS"] = "RECENT"
        } else {
            Write-Host "  Commits since: $commitsSince" -ForegroundColor Red
            Write-Host "  Consider running: run-tests.ps1 -Mode full" -ForegroundColor Red
            $envVars["SESSION_FULL_TEST_COMMITS_SINCE"] = "$commitsSince"
            $envVars["SESSION_FULL_TEST_STATUS"] = "STALE"
        }
    } catch {
        Write-Host "  Could not parse tracking file" -ForegroundColor DarkGray
        $envVars["SESSION_FULL_TEST_STATUS"] = "UNKNOWN"
    }
} else {
    Write-Host "  No full test run recorded" -ForegroundColor Red
    Write-Host "  Run: run-tests.ps1 -Mode full" -ForegroundColor Red
    $envVars["SESSION_FULL_TEST_STATUS"] = "NEVER"
    $envVars["SESSION_FULL_TEST_DATE"] = ""
    $envVars["SESSION_FULL_TEST_COMMIT"] = ""
    $envVars["SESSION_FULL_TEST_COMMITS_SINCE"] = "N/A"
}
Write-Host ""

# --------------------------------------------------------------
# 4b. Most Recent Test Run (any mode)
# --------------------------------------------------------------
...
| Key | Value |
|-----|-------|
| Status | $($envVars["SESSION_FULL_TEST_STATUS"]) |
| Last Run | $($envVars["SESSION_FULL_TEST_DATE"]) |
| At Commit | $($envVars["SESSION_FULL_TEST_COMMIT"]) |
| Commits Since | $($envVars["SESSION_FULL_TEST_COMMITS_SINCE"]) |
